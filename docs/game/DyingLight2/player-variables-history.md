# Dying Light 2 — PlayerVariables research archive

**Status:** Historical / research detail extracted from the main topic doc.  
**Canonical recipe:** [player-variables.md](player-variables.md) (read that first).  
**Do not** use GroupScan or old data AOBs as the primary enable path when the **PlayerState+0xBA8** chain works.

Content below was live session notes (2026-08-01) kept so findings are not lost.

**Naming:** legacy `playerStat` in notes = **PlayerVariables catalog base** (`engine this + 8`).
